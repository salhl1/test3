...
public class ShoppingCartTest {
    ...
    @Test
    public void shouldCalulatePriceFor20PercentDiscount() {
    ...
    public void shouldCalculatePriceForBuyTwoGetOneFree() {
        List<Product> products = asList(new Product(PRICE, "BULK_2_ABCD", PRODUCT),
                new Product(PRICE, "BULK_2_ABCD", PRODUCT),
                new Product(PRICE, "BULK_2_ABCD", PRODUCT));
        ShoppingCart cart = new ShoppingCart(customer, products);
        Order order = cart.checkout();
    ...
    @Test
    public void shouldCalculateLoyaltyPointsForBuyTwoGetOneFree() {
        List<Product> products = asList(new Product(PRICE, "BULK_2_ABCD", PRODUCT),
                new Product(PRICE, "BULK_2_ABCD", PRODUCT),
                new Product(PRICE, "BULK_2_ABCD", PRODUCT));
        ShoppingCart cart = new ShoppingCart(customer, products);
    ...
    @Test
    public void shouldCalculatePriceFor5PercentDiscountOver500() {
        List<Product> products = asList(new Product(6 * PRICE, "DIS_10_ABCD", PRODUCT));
        ShoppingCart cart = new ShoppingCart(customer, products);
        Order order = cart.checkout();

        assertEquals(538.0, order.getTotalPrice(), 0.0);
    }

    @Test
    public void shouldCalculateLoyaltyPointsFor5PercentDiscountOver500() {
        List<Product> products = asList(new Product(6 * PRICE, "DIS_10_ABCD", PRODUCT));
        ShoppingCart cart = new ShoppingCart(customer, products);
        Order order = cart.checkout();

        assertEquals(60, order.getLoyaltyPoints());
    }
}
